...
int Parser::addNode(const Setting &node) {
	...
	if(!node.lookupValue("cache_size", cache_size))
	{
		cout<<"configure file error, can't read cache size"<<endl ;
	}
	else
	{
		if(cache_size<0)
		{
			cerr<<"cache size can't be negative"<<endl ;
		}
		else
		{
			nn->cache_size = cache_size ;
		}
	}
