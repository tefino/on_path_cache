...
void Domain::writeClickFiles(bool montoolstub) {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        if (montoolstub && (nn->running_mode.compare("user") == 0)) {
            click_conf << "require(blackadder_on_path_cache); \n\nControlSocket(\"TCP\",55555);\n\n " << endl << endl;
        } else {
            click_conf << "require(blackadder_on_path_cache);" << endl << endl;
        }
        ...
        if (overlay_mode.compare("mac") == 0)
        {
            click_conf<<"cacheunit"<<"::CacheUnit(globalconf,"<<nn->cache_size<<");"<<endl ;
        }
        else
        {
...
void Domain::startClick() {
    ...
    for (int i = 0; i < network_nodes.size(); i++) {
        ...
        if (nn->running_mode.compare("user") == 0) {
            if (sudo) {
                command = "ssh " + user + "@" + nn->testbed_ip + " \"sudo " + click_home + "bin/click " + write_conf + nn->label + ".conf > /tmp/onpathcache.log 2>&1 &\"";
            } else {
                command = "ssh " + user + "@" + nn->testbed_ip + " \"" + click_home + "bin/click " + write_conf + nn->label + ".conf > /tmp/onpathcache.log 2>&1 &\"";
            }
            cout << command << endl;
            ssh_command = popen(command.c_str(), "r");
            if (ssh_command == NULL) {
...
void Domain::startTM() {
    ...
    pclose(ssh_command);
    /*now start the TM*/
    command = "ssh " + user + "@" + TM_node->testbed_ip + " \"/home/" + "/on_path_cache/TopologyManager/tm " + write_conf + "topology.graphml > /tmp/tm_opc.log 2>&1 &\"";
    cout << command << endl;
    ssh_command = popen(command.c_str(), "r");
    if (ssh_command == NULL) {
